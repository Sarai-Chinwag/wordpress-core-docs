...
6. After a human explicitly approves publication, build the approved artifact and run `WP_DOCS_ALLOW_PUBLISH=1 npm run publish:spacefast`, which invokes `sf publish dist --prebuilt`. Use `npm run publish:spacefast -- --dry-run` first; it invokes `sf publish dist --prebuilt --dry-run`.
